...
The project is honest about model quality: the infrastructure is in place, BC reaches a playable warm start, and PPO mechanics are functioning across 24,000+ training games. The last fixed-seed eval (on the 585-d model) shows 38.1% boss win rate, 20% Act 2 reach rate, and runs past floor 30 including two Act 3 runs (floors 42 and 46). The agent has not yet won a full game; the current research direction (learned deck-building) targets the Act 2 wall. Its progress is tracked with a weight-ratio diagnostic showing the new deck inputs integrating (now ~0.59 of the original inputs' strength); behavior change lags the weight growth, and a clean greedy fixed-seed eval is planned once it matures — leaving a clear next milestone while preserving the project as a systems, tooling, and evaluation portfolio piece.
