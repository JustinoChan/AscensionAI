...
The current public snapshot is honest about model quality: the infrastructure is in place, BC reaches a playable warm start, and PPO mechanics are functioning, but the latest PPO checkpoint has not yet exceeded the heuristic or BC fixed-seed baseline. That makes the project useful as a systems, tooling, and evaluation portfolio piece while leaving a clear next research milestone.
