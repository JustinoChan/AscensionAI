...
The current public snapshot is honest about model quality: the infrastructure is in place, BC reaches a playable warm start, and PPO mechanics are functioning with 16,900+ training games. The latest PPO checkpoint exceeds BC on evaluation but still trails the heuristic baseline and has not yet won, which leaves a clear next research milestone while preserving the project as a systems, tooling, and evaluation portfolio piece.
